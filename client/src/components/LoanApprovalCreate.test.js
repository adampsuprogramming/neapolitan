...
test("UT-59 – This is a unit test to ensure that error message is correctly displayed if data is missing from submission (Loan Approval)", async () => {
  axios.get.mockImplementation((url) => {
    if (url.includes("borrowerquery")) {
      return Promise.resolve({
        data: [
...
test("UT-60 – Testing server error handling", async () => {
  ...
  axios.post.mockRejectedValue(new Error("API Save Error"));
