...
render(<BorrowBase />);
  ...
  await waitFor(() => {
    expect(axios.get).toHaveBeenCalledWith("http://localhost:3000/api/facilities");
  });
...
expect(facilityText).toEqual(["Choose a Facility","Happy Bank Banana Facility", "Tree Bank Fund Banana"]);
